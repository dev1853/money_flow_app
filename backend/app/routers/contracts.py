...
def read_contracts(
    ...
    """
    total_count = crud.contract.get_count_by_workspace(db, workspace_id=current_workspace.id) # ИСПРАВЛЕНО: Используем метод подсчета
    contracts = crud.contract.get_multi_by_workspace(db, skip=skip, limit=limit, workspace_id=current_workspace.id) # ИСПРАВЛЕНО: Передаем workspace_id
...
def update_contract(
    ...
    if not contract or contract.workspace_id != current_workspace.id:
        raise HTTPException(
