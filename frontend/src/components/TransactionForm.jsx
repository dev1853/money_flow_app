...
import Button from './Button'; // Предполагаем, что Button.jsx в той же папке components
import Alert from './Alert';   // Предполагаем, что Alert.jsx в той же папке components
import { API_BASE_URL } from '../apiConfig'; // Путь из frontend/src/components/ до frontend/src/
...
function TransactionForm({
  ...
  const fetchFormData = useCallback(async () => { //
    ...
    try {
      const accountsResponse = await fetch(`${API_BASE_URL}/accounts/?limit=500&is_active=true`, { headers }); //
    ...
    try {
      const articlesResponse = await fetch(`${API_BASE_URL}/articles/`, { headers }); //
    ...
    try {
      const usersResponse = await fetch(`${API_BASE_URL}/users/?limit=500`, { headers }); //
  ...
  const handleSubmit = async (e) => { //
    ...
    if (isEditMode) { //
      ...
      url = `${API_BASE_URL}/transactions/${transactionToEdit.id}/`; //
      method = 'PUT'; //
    } else { //
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4"> {/* */}
        <div>
          ...
          <select id="accountId-form" value={accountId}
                  ...
                  required disabled={isEditMode} className={`${commonInputClasses} ${isEditMode ? 'bg-gray-100 cursor-not-allowed' : ''}`}> {/* */}
            <option value="" disabled={!isEditMode}>-- Выберите счет --</option> {/* */}
          ...
          <select id="ddsArticleId-form" value={ddsArticleId} onChange={(e) => setDdsArticleId(e.target.value)} required className={commonInputClasses}> {/* */}
            ...
            {filteredDdsArticles.map(art => //
              <option
                key={art.id} //
                value={art.id} //
                disabled={art.children && art.children.length > 0 && art.id.toString() !== ddsArticleId} //
              >
