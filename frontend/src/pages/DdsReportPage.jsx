...
import { CheckIcon, ChevronUpDownIcon, DocumentChartBarIcon, ExclamationTriangleIcon, FunnelIcon } from '@heroicons/react/24/outline';
...
const DdsReportPage = () => {
  ...
  const fetchAccountsForFilter = useCallback(async () => {
    ...
    try {
      ...
      setAvailableAccounts([]); // Устанавливаем пустой массив в случае ошибки
    }
  }, [token, isAuthLoading]);
  ...
  // Автоматическая загрузка отчета при монтировании, если даты установлены и пользователь аутентифицирован
  useEffect(() => {
    if (isAuthenticated && !isAuthLoading && startDate && endDate) {
       handleGenerateReport();
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAuthenticated, isAuthLoading]); // Запускаем один раз при готовности auth, даты уже по умолчанию
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-3xl font-bold text-gray-900">Отчет о Движении Денежных Средств</h2>
      </div>

      {/* БЛОК ФИЛЬТРОВ */}
      <div className="p-6 bg-white shadow-xl rounded-2xl">
        <h3 className="text-xl font-semibold text-gray-800 mb-6 border-b pb-3">Параметры отчета</h3>
        <div className="flex flex-wrap items-end gap-x-4 gap-y-4">
          ...
          <div className="flex-shrink-0">
            <button 
              ...
              className={`${commonButtonClasses} text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 w-full sm:w-auto`}
            >
      ...
      {!isLoading && !isAuthLoading && reportData && (
        <div className="bg-white shadow-xl rounded-2xl p-6 mt-8">
          ...
          <div className="mt-8 pt-6 border-t-2 border-gray-300">
            ...
            {/* Здесь можно будет добавить Начальный и Конечный остаток, когда они появятся в reportData */}
          </div>
        </div>
      )}
