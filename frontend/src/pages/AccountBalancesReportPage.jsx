...
const AccountBalancesReportPage = () => {
  ...
  const handleGenerateReport = useCallback(async () => { //
    ...
    // Не начинаем загрузку, если isAuthLoading еще true
  ...
  useEffect(() => { //
    // Автозагрузка при монтировании/изменении даты, если пользователь аутентифицирован и дата валидна
    if (isAuthenticated && !isAuthLoading && reportDate && isValid(reportDate)) {
       handleGenerateReport();
    }
  }, [isAuthenticated, isAuthLoading, reportDate, handleGenerateReport]); // Добавил handleGenerateReport в зависимости
  ...
  return (
    <div className="space-y-6"> {/* Уменьшил общий отступ, PageTitle добавит свой mb-6 */}
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="flex-shrink-0">
            ...
            >
              ...
              <span className="sm:hidden">Отчет</span> {/* Более короткий текст для мобильных */}
            </Button>
          </div>
        </div>
        {/* Отображение ошибки теперь Alert компонентом */}
        {error && !isLoading && ( //
      ...
      {/* Локальный лоадер для загрузки самого отчета */}
      {isLoading && !isAuthLoading && (
         <Loader message="Формирование отчета..." containerClassName="text-center py-10" />
      )}

      {!isLoading && !isAuthLoading && reportData && ( //
        <div className="bg-white shadow-xl rounded-2xl p-6"> {/* Убрал mt-8, т.к. общий space-y-6 есть */}
      ...
      {/* Сообщение "Отчет не сформирован" теперь через EmptyState */}
